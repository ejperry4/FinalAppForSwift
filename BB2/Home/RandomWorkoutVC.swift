...
class RandomWorkoutVC: UIViewController {
    ...
    var drillTimes = [0.30, 0.30, 0.45, 0.30, 0.30, 0.30, 0.30, 0.30, 0.30, 0.30, 0.30, 0.45, 0.30, 0.30, 0.45, 0.45, 0.45, 0.45, 0.30, 0.30, 0.45, 0.45, 0.45, 1.80, 1.80, 4.00, 4.00, 0.60, 0.60, 4.00, 0.60, 0.60, 0.60, 0.60, 0.60, 0.60, 0.60, 0.60]
    var drills: [Drills] = []
    override func viewDidLoad() {
        super.viewDidLoad()
        print(workout)
        if segueName == "FromSpecificWorkout" {
            for index in workout {
                drills.append(Drills(name: drillNames[index], description: drillDescriptions[index], gif: gifNames[index], time: drillTimes[index]))
            }
            
        } else if segueName == "FromGenerateWorkout" {
            for index in workout {
                drills.append(Drills(name: drillNames[index], description: drillDescriptions[index], gif: gifNames[index], time: drillTimes[index]))
            }
        } else {
...
extension  RandomWorkoutVC: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return numberOfRows
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath)
        cell.textLabel?.text = drillNames[workout[indexPath.row]]
        return cell
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "RandomVCToDrillVC" {
